...
class Agent(ABC):
    ...
    def make_move(self) -> int:
        move, nodes_explored = self._get_move(start_from=self.game.state.last_hop_to)
        if move is not None:
            _action(action=move, state=self.game.state)
        else:
            self.game.end_turn()
...
def _action(action: Action, state: GameState):
