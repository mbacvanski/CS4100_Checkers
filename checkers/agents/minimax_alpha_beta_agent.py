...
class MinimaxAlphaBetaAgent(Agent):
    ...
    def minimax(self, starting_state: Node, depth=10,
                alpha=float('-inf'), beta=float('inf')) -> Tuple[float, Union[Action, None]]:
    ...
    def run_max(self, state: Node, alpha: float, beta: float) -> Tuple[float, Union[Action, None]]:
        max_value = float('-inf')
        max_action = None

        for action in state.next_actions():
            new_game_state = state.next_node(action)
            value, _ = self.minimax(new_game_state, self.depth_limit, alpha=alpha, beta=beta)
    ...
    def run_min(self, state: Node, alpha: float, beta: float) -> Tuple[float, Union[Action, None]]:
        min_value = float('inf')
        min_action = None
